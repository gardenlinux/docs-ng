...
def transform_directory_structure(
    ...
    # For sphinx structure: clean the target directory first so stale source
    # files from previous runs do not accumulate alongside the built Markdown.
    if structure_map == "sphinx" and target_path.exists():
        print(f"  Cleaning target directory for sphinx build: {target_path}")
    ...
    if isinstance(structure_map, dict):
        ...
        # Files generated by Sphinx that are not useful in VitePress
        _sphinx_artifacts = {"genindex.md", "py-modindex.md", "search.md"}
        for item in source_path.glob("*"):
            if structure_map == "sphinx" and item.name in _sphinx_artifacts:
                print(f"  Skipping Sphinx artifact: {item.name}")
                continue
            # For sphinx structure: skip Markdown files that already have a
            # github_target_path frontmatter field — they were placed at their
            # canonical VitePress location by copy_targeted_docs (Step 1) and
            # must not be duplicated here, as duplicate HTML anchor IDs across
            # two pages break VitePress's MiniSearch indexer.
            if structure_map == "sphinx" and item.is_file() and item.suffix == ".md":
                try:
                    content = item.read_text(encoding="utf-8")
                    frontmatter, _ = parse_frontmatter(content)
                    if frontmatter and "github_target_path" in frontmatter:
                        print(f"  Skipping targeted file (already placed): {item.name}")
                        continue
                except Exception:
                    pass
